...
| [TP-005](TP-005-network-share.md) | 2026-08-03 | milestone, network share | SMB mechanics pass over loopback. **Found a critical defect unrelated to SMB: a commit silently discards another client's work (F-B-04)**, and corrects TP-002's reading of its own concurrency case. A real-NAS run stays open, clock skew named as its first target |
| [TP-006](TP-006-two-artists.md) | 2026-08-03 | S5a transfer half | Pass, no defects. Two artists on one store with real `.blend` files: disjoint edits merge silently, a contested asset yields an openable conflict copy, a deletion never beats an edit. Blender was not live during the run |
